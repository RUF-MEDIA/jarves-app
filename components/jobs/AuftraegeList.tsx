...
const columnOrder: (keyof Auftrag)[] = ['autogeneratedNr', 'jobtitel', 'unternehmenId', 'job', 'status', 'einsatzort', 'erstelltAm', 'betreuerId'];
const standardColumns: Set<keyof Auftrag> = new Set(columnOrder);
...
// BulkActionSidebarAuftraege Component
const BulkActionSidebarAuftraege: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  selectedRows: string[];
  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        <p>{selectedRows.length} Aufträge ausgewählt</p>
...
const createColumns = (data: Auftrag[]): ColumnDef<Auftrag>[] => {
  const allFields = new Set<keyof Auftrag>([...columnOrder, ...Array.from(new Set(data.flatMap((item) => Object.keys(item) as (keyof Auftrag)[])))]);
  ...
  return Array.from(allFields).map(
    (key): ColumnDef<Auftrag> => ({
      ...
      cell: ({ row }) => {
        ...
        if (key === 'unternehmenId' && value) {
          return (
            <Link href={`/kunden/${value}`} className="text-blue-600 hover:underline">
...
export function AuftrageList() {
  ...
  useEffect(() => {
    const fetchAuftraege = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('/api/getAuftraege');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    columns.forEach((column) => {
      if ('accessorKey' in column && typeof column.accessorKey === 'string') {
        initialVisibility[column.accessorKey] = standardColumns.has(column.accessorKey as keyof Auftrag);
      }
    });
  ...
  const filteredData = useMemo(() => {
    return data.filter((auftrag) => {
      if (jobFilter) {
        return auftrag.job?.toLowerCase().includes(jobFilter.toLowerCase());
      }
      return true;
    });
  }, [data, jobFilter]);
  ...
  const handleBulkUpdate = async (updates: any) => {
    ...
    try {
      const response = await fetch('/api/bulkUpdateAuftraege', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        const updatedData = await fetch('/api/getAuftraege').then((res) => res.json());
        setData(updatedData);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch('/api/bulkDeleteAuftraege', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Input placeholder="Job suchen..." value={jobFilter} onChange={(e) => setJobFilter(e.target.value)} className="max-w-sm" />
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
