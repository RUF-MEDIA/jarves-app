...
interface Unternehmen {
  ...
  // Optional dynamische Felder können hier hinzugefügt werden
...
const StatusSelectFilter: React.FC<{ column: Column<Unternehmen, unknown>; options: string[] }> = ({ column, options }) => {
...
const StarRatingFilter: React.FC<{ column: Column<Unternehmen, unknown> }> = ({ column }) => {
...
const renderCell = (key: string, row: Row<Unternehmen>) => {
...
export function ContactList() {
  ...
  const handleBulkUpdate = async () => {
    ...
    const updates: Partial<Pick<Unternehmen, 'status' | 'kategorie' | 'betreuerId' | 'unternehmensverknuepfung'>> = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setData((prevData) => prevData.map((unternehmen) => (selectedIds.includes(unternehmen.id) ? { ...unternehmen, ...updates } : unternehmen)));
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmDelete = window.confirm('Sind Sie sicher, dass Sie die ausgewählten Unternehmen löschen möchten?');
    ...
    try {
      ...
      if (response.ok) {
        ...
        setData((prevData) => prevData.filter((unternehmen) => !selectedIds.includes(unternehmen.id)));
        setIsBulkActionSidebarOpen(false);
        setRowSelection({});
      } else {
